...
const AWSClientService = () => {
    ...
    const hardRemoveUser = async (arguments) => {
        ...
        try {
            ...
            // 1. Delete password
            var data = await iamClient
                .send(new DeleteLoginProfileCommand(params))
                .then((res) => {
                    console.log(res);
                })
                .catch((err) => {
            ...
            var keys = await iamClient
                .send(new ListAccessKeysCommand(params))
                .catch((err) => {
                    console.error("Error", err);
                });

            for (let i = 0; i < keys.AccessKeyMetadata.length; i++) {
                data = await iamClient
                    .send(
            ...
            keys = await iamClient
                .send(new ListSigningCertificatesCommand(params))
                .catch((err) => {
                    console.error("Error", err);
                });
            for (let i = 0; i < keys.Certificates.length; i++) {
                data = await iamClient
                    .send(
                        new DeleteSigningCertificateCommand({
                            CertificateId: keys.Certificates[i].CertificateId,
                            UserName: arguments[0],
                        })
                    )
                    .catch((err) => {
            ...
            // 4. Delete ssh keys
            keys = await iamClient
                .send(new ListSSHPublicKeysCommand(params))
                .catch((err) => {
                    console.error("Error", err);
                });
            console.log(keys);
                ...
                data = await iamClient
                    .send(
                        new DeleteSSHPublicKeyCommand({
                            SSHPublicKeyId:
                                keys.SSHPublicKeys[i].SSHPublicKeyId,
                            UserName: arguments[0],
                        })
                    )
                    .catch((err) => {
            ...
            keys = await iamClient
                .send(new ListServiceSpecificCredentialsCommand(params))
                .catch((err) => {
                    console.error("Error", err);
                });
            console.log(keys);
                    ...
                    .send(
                        new DeleteServiceSpecificCredentialCommand({
                            ServiceSpecificCredentialId:
                                keys.ServiceSpecificCredentials[i]
                                    .ServiceSpecificCredentialId,
                            UserName: arguments[0],
                        })
                    )
            ...
            keys = await iamClient
                .send(new ListMFADevicesCommand(params))
                .catch((err) => {
                    console.error("Error", err);
                });
            console.log(keys);
            for (let i = 0; i < keys.MFADevices.length; i++) {
                data = await iamClient
                    .send(
                        new DeactivateMFADeviceCommand({
                            SerialNumber: keys.MFADevices[i].SerialNumber,
                            UserName: arguments[0],
                        })
                    )
                    .catch((err) => {
                ...
                data = await iamClient
                    .send(
                        new DeleteVirtualMFADeviceCommand({
                            SerialNumber: keys.MFADevices[i].SerialNumber,
                        })
                    )
                    .catch((err) => {
            ...
            keys = await iamClient
                .send(new ListUserPoliciesCommand(params))
                .catch((err) => {
                    console.error("Error", err);
                });
            console.log(keys);
            ...
            // 8. Detach policies
            keys = await iamClient
                .send(new ListAttachedUserPoliciesCommand(params))
                .catch((err) => {
                    console.error("Error", err);
                });
                ...
                data = await iamClient
                    .send(
                        new DetachUserPolicyCommand({
                            PolicyArn: keys.AttachedPolicies[i].PolicyArn,
                            UserName: arguments[0],
                        })
                    )
                    .catch((err) => {
            ...
            keys = await iamClient
                .send(new ListGroupsForUserCommand(params))
                .catch((err) => {
                    console.error("Error", err);
                });
                ...
                data = await iamClient
                    .send(
                        new RemoveUserFromGroupCommand({
                            GroupName: keys.Groups[i].GroupName,
                            UserName: arguments[0],
                        })
                    )
                    .catch((err) => {
            ...
            // 10. delete user
            data = await iamClient
                .send(new DeleteUserCommand(params))
                .catch((err) => {
                    console.error("Error", err);
                });
    ...
    const quarantineUser = async (arguments) => {
        try {
            console.log(
                "Deleting a user from all their groups and putting them into a quarantine group"
            );
            console.log(arguments[0]);
            const params = {
                UserName: arguments[0],
            };
            const user = await iamClient.send(new GetUserCommand(params));
            ...
            const data2 = await iamClient.send(
                new AddUserToGroupCommand({
                    GroupName: "Quarantine",
                    UserName: arguments[0],
                })
            );
            console.log("Success", JSON.stringify({ data1, data2 }));
            return { data1, data2 };
        } catch (err) {
            console.log("Error", err);
            return err["Code"];
