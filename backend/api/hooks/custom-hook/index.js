...
module.exports = function defineCustomHookHook(sails) {
  ...
  return {
    ...
    initialize: async function () {
      ...
      if (isMissingStripeConfig || isMissingMailgunConfig) {
        ...
        let missingFeatureText = isMissingStripeConfig && isMissingMailgunConfig ? 'billing and email' : isMissingStripeConfig ? 'billing' : 'emailAddress';
        let suffix = '';
...
---------------------------------------------------------------------${suffix}`);
      ...
      if(!fs.existsSync('.env')){
        throw new Error('Ensure a .env file exists.');
      }
    },
